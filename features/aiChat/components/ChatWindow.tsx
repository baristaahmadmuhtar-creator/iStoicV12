...
import { 
    ...
    Download, Maximize2, Network, Eye, Code as CodeIcon, Play,
    BrainCircuit
} from 'lucide-react';
...
const ThinkingAccordion = ({ content, isActive }: { content: string, isActive?: boolean }) => {
    ...
    // Auto-expand only when strictly active (generating thought)
    useEffect(() => { 
    ...
    return (
        <div className={`my-3 rounded-xl overflow-hidden border transition-all duration-500 w-full group/thought ${
            isActive 
            ? 'border-indigo-500/40 bg-indigo-500/10 shadow-[0_0_25px_rgba(99,102,241,0.15)]' 
            ...
            >
                <div className="flex items-center gap-3">
                    <div className={`p-1.5 rounded-lg ${isActive ? 'bg-indigo-500 text-white animate-pulse' : 'bg-white/10 text-neutral-400'}`}>
                        <BrainCircuit size={14} />
                    </div>
                    <div className="flex flex-col items-start leading-none gap-1">
                        <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${isActive ? 'text-indigo-400' : 'text-neutral-500'}`}>
                            {isActive ? 'NEURAL_PROCESS_ACTIVE' : 'COGNITIVE_TRACE'}
                        </span>
                        {isActive && <span className="text-[8px] text-indigo-300/70 font-mono">Exploring vector space...</span>}
                    </div>
                </div>
                <div className={`p-1.5 rounded-lg transition-all ${isExpanded ? 'bg-indigo-500/20 text-indigo-400' : 'text-neutral-500'}`}>
            ...
            <div className={`transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden ${isExpanded ? 'max-h-[800px] opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="relative border-t border-white/5 bg-[#050505]">
                    {/* Matrix Rain Effect Strip */}
                    <div className="absolute top-0 left-0 bottom-0 w-[2px] bg-gradient-to-b from-indigo-500 via-purple-500 to-transparent opacity-50"></div>
                    
                    <div className="p-4 pl-6 overflow-x-auto custom-scroll">
                        <pre className="text-[10px] font-mono leading-[1.8] text-neutral-400 whitespace-pre-wrap font-medium tracking-tight">
                            {content}
                            {isActive && <span className="inline-block w-2 h-4 ml-1 bg-indigo-500 animate-pulse align-middle"></span>}
                        </pre>
                    </div>
                </div>
            </div>
...
const MessageBubble = memo(({ 
    ...
    const { thought, content, imgPrompt } = useMemo(() => {
        ...
        // Enhanced DeepSeek <think> block parsing
        // Supports streaming where </think> might not be present yet
        if (text.includes('<think>')) {
            ...
            if (hasClosing) {
                ...
                // Streaming: The thought is everything after <think>, content is empty for now
