...
import { 
    ...
    AlertTriangle, Server, Network, Lock, Info, ChevronRight
} from 'lucide-react';
...
export const ModelPicker: React.FC<ModelPickerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'AUTO' | 'GOOGLE' | 'GROQ' | 'DEEPSEEK'>('AUTO');
  const [statuses, setStatuses] = useState<ProviderStatus[]>([]);
  const { features } = useFeatures();
  const [thinkingBudget, setThinkingBudget] = useLocalStorage<number>('thinking_budget', 4096);
  ...
  // Grupkan model dari katalog
  const tabs = {
    'AUTO': {
        icon: <CircuitBoard size={16}/>,
        label: 'HYDRA',
        sub: features.OMNI_RACE ? 'OMNI-RACE' : 'DISABLED',
        desc: 'Parallel execution engine. Winner takes all.',
    ...
    'GOOGLE': { 
        ...
        label: 'GEMINI', 
        sub: 'DEEPMIND',
        desc: 'Native Multimodal. Best balance of speed & intellect.',
    ...
    'GROQ': { 
        ...
        label: 'GROQ', 
        sub: 'LPU SPEED',
        desc: 'Instant inference using Llama 3 & DeepSeek Distill.',
    ...
    'DEEPSEEK': { 
        ...
        label: 'DEEPSEEK', 
        sub: 'REASONING',
        desc: 'Advanced Chain-of-Thought (R1 & V3).',
    ...
    <div className="fixed inset-0 z-[2000] flex items-end md:items-center justify-center p-0 md:p-4 animate-fade-in font-sans">
      ...
      <div className="relative z-10 w-full md:max-w-5xl h-[92vh] md:h-[85vh] bg-[#050505] rounded-t-[32px] md:rounded-[24px] border-t md:border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden ring-1 ring-white/5 animate-slide-up">
        ...
        <div className="h-16 md:h-14 border-b border-white/5 bg-white/[0.02] flex items-center justify-between px-6 shrink-0">
            ...
            <button onClick={onClose} className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-neutral-400 hover:text-white transition-all border border-white/5 active:scale-95"><X size={16} /></button>
        </div>
        ...
        <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
                    ...
                    return (
                        <button
                            ...
                            className={`
                                ...
                                ${isActive ? 'bg-white/5 border-white/10 shadow-inner' : 'bg-transparent border-transparent hover:bg-white/[0.02] hover:border-white/5'}
                        ...
                        >
                            ...
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all shrink-0 ${isActive ? `${data.bg} ${data.accent}` : 'bg-white/5 text-neutral-500 group-hover:text-white'}`}>{data.icon}</div>
                            <div className="text-left flex-1 min-w-0 hidden md:block">
                                <div className="flex items-center justify-between">
                                    <span className={`text-[10px] font-black uppercase tracking-widest ${isActive ? 'text-white' : 'text-neutral-400'}`}>{data.label}</span>
                                    {key !== 'AUTO' && <div className={`w-1.5 h-1.5 rounded-full ${isHealthy ? 'bg-emerald-500' : 'bg-red-500'}`}></div>}
                                </div>
                                <span className="text-[8px] font-bold text-neutral-600 uppercase tracking-wide block">{data.sub}</span>
                            </div>
                        </button>
                    );
                })}
            ...
            <div className="flex-1 bg-[#09090b] relative flex flex-col min-h-0">
                ...
                <div className="p-6 md:p-8 pb-4 shrink-0 relative z-10">
                    <h2 className={`text-3xl md:text-4xl font-black italic tracking-tighter uppercase text-white`}>{currentTab.label} <span className="opacity-50 text-xl not-italic tracking-widest">// MODULE</span></h2>
                    <p className="text-[10px] text-neutral-400 font-mono uppercase tracking-[0.2em] border-l-2 border-white/10 pl-3 leading-relaxed max-w-lg mt-2">{currentTab.desc}</p>
                </div>

                <div className="flex-1 overflow-y-auto custom-scroll p-4 md:p-8 pt-0 relative z-10 pb-20">
                    <div className="grid grid-cols-1 gap-4">
                        {/* Thinking Budget Slider for Gemini */}
                        {activeTab === 'GOOGLE' && (
                            <div className="mb-4 p-4 rounded-xl bg-white/5 border border-white/10">
                                <div className="flex justify-between items-center mb-2">
                                    <label className="text-[9px] font-black uppercase tracking-widest text-neutral-400 flex items-center gap-2">
                                        <Brain size={12} className="text-blue-400"/> Thinking Budget
                                    </label>
                                    <span className="text-[10px] font-mono text-white">{thinkingBudget} tokens</span>
                                </div>
                                <input 
                                    type="range" 
                                    min="1024" 
                                    ...
                                    className="w-full h-1.5 bg-black/20 rounded-lg appearance-none cursor-pointer accent-blue-500" 
                                />
                        ...
                        {currentTab.models.map(model => (
                            <ArchitectureCard key={model.id} model={model} isActive={model.id === activeModelId} onClick={() => onSelectModel(model.id)} accent={currentTab.accent} statuses={statuses} />
                        ))}
                    </div>
                </div>
            </div>
...
const ArchitectureCard: React.FC<{ model: any, isActive: boolean, onClick: () => void, accent: string, statuses: ProviderStatus[] }> = ({ model, isActive, onClick, accent, statuses }) => {
  ...
  return (
    <button onClick={onClick} className={`relative w-full text-left transition-all duration-300 group overflow-hidden rounded-[20px] border p-[1px] active:scale-[0.99] ${isActive ? 'bg-white/[0.03]' : 'bg-transparent hover:bg-white/[0.01]'} ${isActive ? borderColorClass : 'hover:border-white/10'}`}>
        <div className="bg-[#0c0c0e] rounded-[19px] p-5 relative z-10 h-full flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1 flex flex-col justify-between">
                <div>
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                        <span className={`text-base md:text-lg font-black uppercase tracking-tight ${isActive ? 'text-white' : 'text-neutral-300 group-hover:text-white'} transition-colors`}>{model.name}</span>
                        {isAuto && <span className="px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 text-[8px] font-black tracking-widest uppercase animate-pulse">OMNI-RACE</span>}
                        {isActive && <div className={`px-2 py-0.5 rounded bg-white/10 border border-white/10 text-[8px] font-bold uppercase tracking-wider text-white flex items-center gap-1`}><CheckCircle2 size={10} className={accent} /> ACTIVE</div>}
                    </div>
                    <p className="text-[10px] text-neutral-500 font-medium leading-relaxed pr-4 line-clamp-2 group-hover:text-neutral-400 transition-colors">{model.description}</p>
                </div>
                <div className="flex items-center gap-3 mt-4 flex-wrap">
                    <StatusBadge status={statusType as any} cooldown={providerStatus?.cooldownRemaining} />
                    {model.specs.speed === 'THINKING' && <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-purple-500/10 border border-purple-500/20 text-purple-400"><Brain size={10} /><span className="text-[7px] font-black uppercase tracking-wider">REASONING</span></div>}
                </div>
            </div>
            <div className="w-full md:w-48 shrink-0 flex flex-row md:flex-col justify-between md:justify-center gap-4 border-t md:border-t-0 md:border-l border-white/5 pt-4 md:pt-0 md:pl-6">
                <div className="flex-1 md:w-full"><TechStatBar label="INTELLIGENCE" value={model.specs.intelligence} max={10} color={model.specs.intelligence >= 9.5 ? 'text-purple-400' : 'text-blue-400'} /></div>
                <div className="flex flex-col gap-1 items-end md:items-start"><span className="text-[7px] font-black text-neutral-500 uppercase tracking-widest">VELOCITY</span><div className="flex items-center gap-2"><Gauge size={12} className={model.specs.speed === 'INSTANT' ? 'text-yellow-400' : 'text-neutral-400'} /><span className={`text-[9px] font-black uppercase ${model.specs.speed === 'INSTANT' ? 'text-yellow-400' : 'text-neutral-300'}`}>{model.specs.speed}</span></div></div>
            </div>
        </div>
    </button>
  );
