    ...
    return (
        <div 
            ...
            className={`
                ...
                hover:-translate-y-2 active:scale-[0.98] group flex flex-col justify-between p-8 md:p-10
                ${className}
            `}
        >
            ...
            <div className="relative z-10">
                ...
                <h3 className="text-4xl md:text-5xl font-black text-black dark:text-white uppercase italic tracking-tighter leading-[0.85] mb-4">{title}</h3>
            ...
            <div className="relative z-10 mt-auto pt-8 border-t border-black/5 dark:border-white/5 flex items-center gap-3 opacity-60 group-hover:opacity-100 transition-opacity">
...
const DashboardView: React.FC<DashboardProps> = ({ onNavigate }) => {
    ...
    const handleNavNotes = () => {
        debugService.logAction(UI_REGISTRY.DASHBOARD_CARD_NOTES, FN_REGISTRY.NAVIGATE_TO_FEATURE, 'NOTES');
        onNavigate('notes');
    };
            ...
            <div className="max-w-[1400px] mx-auto w-full space-y-8 md:space-y-10">
                ...
                <header className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-10 animate-slide-up pb-2">
                    <div className="space-y-4 md:space-y-6 flex-1">
                        ...
                        <h1 className="text-[13vw] md:text-[7rem] xl:text-[8rem] font-black italic tracking-tighter text-black dark:text-white leading-[0.85] uppercase drop-shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-5 w-full xl:w-auto min-w-[340px]">
                        <StatBox 
                            label={t.nodes} 
                ...
                <div className="grid grid-cols-1 md:grid-cols-6 lg:grid-cols-12 gap-6 md:gap-8">
                    ...
                    <div className="md:col-span-6 lg:col-span-8 bg-zinc-100 dark:bg-[#0f0f11] rounded-[48px] border border-black/5 dark:border-white/5 p-8 md:p-10 flex flex-col justify-between animate-slide-up lg:aspect-[1.618/1]" style={{ animationDelay: '400ms' }}>
                        <div className="flex items-center justify-between mb-8">
                            <h3 className="text-3xl font-black uppercase italic tracking-tighter text-black dark:text-white flex items-center gap-4">
                                <Clock size={32} className="text-neutral-400" /> {t.recent}
                            </h3>
                            <button onClick={handleNavNotes} className="w-14 h-14 rounded-full bg-white/5 hover:bg-accent hover:text-black flex items-center justify-center transition-all border border-black/5 dark:border-white/5">
                        ...
                        <div className="space-y-4">
                            {notes.slice(0, 3).map((note, i) => (
                                <div key={note.id} onClick={handleRecentLogClick} className="group flex items-center gap-6 p-5 rounded-[28px] bg-white dark:bg-black/20 border border-black/5 dark:border-white/5 hover:border-accent/30 hover:bg-white dark:hover:bg-white/5 transition-all cursor-pointer shadow-sm hover:shadow-md hover:-translate-y-0.5">
                    ...
                    <div className={`md:col-span-6 lg:col-span-4 p-8 md:p-10 rounded-[48px] border transition-all duration-500 flex flex-col justify-between group shadow-lg animate-slide-up h-full ${isVaultUnlocked ? 'bg-white dark:bg-[#0f0f11] border-accent/50 shadow-[0_0_60px_rgba(var(--accent-rgb),0.1)]' : 'bg-zinc-200 dark:bg-[#0a0a0b] border-transparent'}`} style={{ animationDelay: '500ms' }}>
                        <div>
                            ...
                            <h3 className="text-4xl font-black uppercase italic tracking-tighter leading-none text-black dark:text-white">{t.control}</h3>
