...
const WRITER_PRESETS = [
    ...
    { id: 'SUMMARIZE', label: 'SUMMARIZE', icon: <MinimizeIcon size={12}/>, prompt: "Summarize the key points into a concise list or paragraph." },
];
...
export const AdvancedEditor: React.FC<AdvancedEditorProps> = ({ 
  ...
  useEffect(() => {
      ...
      checkOverlayState();
      window.addEventListener('resize', checkOverlayState);
      return () => {
          window.removeEventListener('resize', checkOverlayState);
          document.body.style.overflow = '';
      };
  }, [showHanisahOverlay, isFocusMode, isTaskPanelOpen]);
  ...
  useEffect(() => {
      if (tasks !== initialTasks || tags !== initialTags) {
          triggerAutoSave();
      }
  }, [tasks, tags, triggerAutoSave, initialTasks, initialTags]);
  ...
  const toggleDictation = () => {
    ...
    r.onerror = (e: any) => setIsDictating(false);
    r.onresult = (e: any) => {
