...
export class NeuralLinkService {
            ...
            const sessionPromise = ai.live.connect({
                ...
                callbacks: {
                    onopen: () => {
                        ...
                        // Keep-alive / Monitor
                        // This prevents audio context suspension when backgrounded on some mobile browsers
                    ...
                    onerror: (e) => {
                        ...
                        // Only report error if we were connected or connecting
    ...
    private async handleServerMessage(msg: LiveServerMessage, sessionPromise: Promise<any>) {
        ...
        // Don't kill the audio buffer too aggressively to prevent cutting off words
        if (msg.serverContent?.interrupted) {
            // Optional: You might want to fade out instead of hard stop, 
            // but for immediate response, clearing is standard.
            // We only clear future queued sources, let current one finish potentially? 
            // No, standard is immediate stop.
