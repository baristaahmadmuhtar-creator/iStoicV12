...
// Helper: Extract valid JSON from mixed text (Robust against Markdown & System Logs)
const extractJson = <T>(text: string | undefined, fallback: T): T => {
    ...
    try {
        ...
        // 2. Try parsing refined text
        try { return JSON.parse(clean); } catch (e) {}

        // 3. Regex Search for Array [...] or Object {...}
        // This bypasses "Here is the JSON:" prefixes or "> System Error" suffixes
...
export const NOTE_AGENTS = {
    ...
    async runOrganizer(notes: Note[], persona: 'hanisah' | 'stoic'): Promise<Partial<Note>[]> {
        const snippet = notes.map(n => ({
            id: n.id,
            title: n.title,
            content: n.content.slice(0, 300), // Reduce tokens
            tags: n.tags,
            created: n.created
        }));
    ...
    async runInsight(notes: Note[]): Promise<string> {
        ...
        const response = await HANISAH_KERNEL.execute(prompt, 'gemini-3-flash-preview');
        // Clean up system log artifacts if present in text response
    ...
    async runActionExtractor(notes: Note[]): Promise<{ text: string, sourceNoteId: string }[]> {
        // Filter only notes updated in last 7 days to save tokens
